    ...
    with conn.cursor() as cur:
        cur.execute("""
            CREATE TABLE IF NOT EXISTS ulb_jobs (
                ...
                group_room      SMALLINT NOT NULL DEFAULT 0,
...
def get_enabled_jobs() -> list[dict]:
    conn = get_connection()
    with conn.cursor() as cur:
        cur.execute("SELECT * FROM ulb_jobs WHERE enabled = 1 ORDER BY id")
        rows = cur.fetchall()
    conn.close()
    return rows
...
def delete_job(job_id: int) -> None:
    conn = get_connection()
    with conn.cursor() as cur:
        cur.execute("DELETE FROM ulb_jobs WHERE id = %s", (job_id,))
    conn.commit()
    conn.close()
...
def toggle_job(job_id: int) -> bool:
    ...
    conn = get_connection()
    with conn.cursor() as cur:
        cur.execute("UPDATE ulb_jobs SET enabled = 1 - enabled WHERE id = %s", (job_id,))
        conn.commit()
        cur.execute("SELECT enabled FROM ulb_jobs WHERE id = %s", (job_id,))
        row = cur.fetchone()
    conn.close()
...
def disable_job(job_id: int) -> None:
    conn = get_connection()
    with conn.cursor() as cur:
        cur.execute("UPDATE ulb_jobs SET enabled = 0 WHERE id = %s", (job_id,))
    conn.commit()
    conn.close()
